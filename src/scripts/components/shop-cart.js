...
const errorTexts = {
  en: {
    add: {
      default: "<h5>Failed to add to cart!</h5><p>Try again, and if this problem persists, contact us!</p>",
      notEnough: "<h5>Could not add to cart!</h5><p>You tried to add too many items to your cart, please try a smaller quantity.</p>",
    },
    remove: {
      default: "<h5>Could not remove item from cart!</h5><p>This item is not in your cart!</p>"
    },
    update: {
      default: "<h5>Failed to update cart!</h5><p>Try again, and if this problem persists, contact us!</p>",
      notEnough: "<h5>Added maximum quantity of items!</h5><p>You tried to add too many items to your cart, please try a smaller quantity.</p>"
    }
  }
}
...
class ShopCart {
  ...
  async init() {
    ...
    this.setupListeners();
    this.setupNote();
    if (this.ready.currencies && this.ready.templates && this.ready.listeners) {
  ...
  async initialRender() {
    var self = this;
    return self.getCartState().then(resp=> {
      self.lastState = resp; //Setup initial lastState
      return true;
    }).catch(err=>{self.okay = false;   return false});
  }
  
  ///TEMPLATES
  setupTemplates() {
    this.templates = window.cartTranslateTemplates;
  ...
  renderCurrency(price, activeCurrency) {
    ...
    var combinedCurrency = '<span class=\"currency\">'  + matchedCurrency.symbol_native+ '</span>';
    ...
    var renderedPrice = [priceString.slice(0, -2), '.', priceString.slice(-2)].join('')
    return combinedCurrency + '<span class=\"price-number\">' + renderedPrice + '</span>';
  }
  ...
  setupListeners() {
    this.listenerCache =[];
    var self = this;
    if (!self.ready.currencies || !self.ready.templates) { console.log('somethings not ready!'); return false;}
  ...
  setupSingleListener(item, itemIndex) {
    ...
    if (quantitySelect) {
      ...
      quantitySelect.parentNode.querySelector('.adjust-minus').addEventListener('click', () => {
        item.classList.add(classes.itemPre)
      })
      quantitySelect.parentNode.querySelector('.adjust-plus').addEventListener('click', () => {
        item.classList.add(classes.itemPre)
      })
    }
  ...
  watchQuantity(input) {
      this.quantityWatchers.push({id: input.id, cachedVal: input.value, el: input});
  }
  checkQuantityChange(watcher) {
    if(watcher.el.value != watcher.cachedVal) {
      ...
      this.updateCartItem(
        {variantId: watcher.el.closest('[data-cart-item]').getAttribute('data-cart-item'), key:  watcher.el.id.replace('updates_', '')},{"quantity":  parseInt(watcher.el.value)}
      ).then(resp => {
    ...
    if (changes) {
      ...
      if (changes.length >=1) {
        changes.forEach(change=> {
          switch (change.type) {
            case "add"    : self.renderAddedItem(change.item)   ; break;
            case "update" : self.renderUpdatedItem(change.item) ; break;
            case "remove" : self.renderRemovedItem(change.item) ; break;
          }
        })
  ...
  getCartChanges() {
    if(!this.state) {
      throw new Error('Error at getCartChanges: Cart not properly initialized!')
    }
    ...
    if (newItems && newItems.length > 0) {
      ...
      if (!oldItems) {
        ...
        newItems.forEach(newItem=> {
          const matchedItem = this.lastState.items.find((oldItem)=> {return oldItem.variant_id == newItem.variant_id})
          if (matchedItem) {
            if (matchedItem.quantity != newItem.quantity || matchedItem.price != newItem.price ) {
      ...
      oldItems.forEach(oldItem=> {
        var matchedItem = newItems.find(newItem => { return oldItem.variant_id == newItem.variant_id })
        if (!matchedItem) {
  ...
  renderCartSubtotal() {
    document.querySelectorAll(selectors.cartSubtotal).forEach(sub=> {
      sub.innerHTML = this.renderCurrency(this.state.total_price  , this.currency);
      this.flash(sub);
    })
  }
  ...
  renderRemovedItem(item, delay) {
    ...
    console.log("Removing item " + item.variant_id)
    setTimeout(() => {
  ...
  async prepareAddedItem(item) {
    ...
    if (indexOfEnding < 0) { indexOfEnding = item.image.indexOf('.jpg') } 
    if (indexOfEnding < 0) { indexOfEnding = item.image.indexOf('.gif') } 
    if (indexOfEnding < 0) { indexOfEnding = item.image.indexOf('.jpeg') } 
    if (indexOfEnding < 0) { indexOfEnding = item.image.indexOf('.webm') } 
    
    var newImg = [item.image.substring(0, indexOfEnding), '_200x200', item.image.substring(indexOfEnding)].join('')
    var scope = {
      itemUrl: item.url,
    ...
    return Mustache.render(this.templates.item, scope)
  }
  ...
  renderAddedItem(item) {
    ...
    this.prepareAddedItem(item).then(rendered=> {
      ...
      this.watchItem(newEl, this.itemList.querySelectorAll(selectors.cartItem).length )
      this.renderLoop(newEl, classes.itemAdding);
      var newQuant = new QuantitySelector(newEl.querySelector('.quantity-select'))
  ...
  renderUpdatedItem(item) {
    ...
    if (targTotal) {
      targTotal.innerHTML = this.renderCurrency(item.final_line_price, this.currency);
      targetEl.addEventListener(animationEvent, () => {
  ...
  renderFallbackMessage() {
   if (this.state.items.length > 0) {
     ...
     this.el.querySelector(selectors.fallbackWrap).setAttribute('aria-hidden', true)
    } else {
      this.itemList.classList.remove('has-items');
      this.el.querySelector(selectors.fallbackWrap).removeAttribute('aria-hidden');
   }
  }
  ...
  async getLineItem(args) {
    ...
    if (lineItem) return { variantId: args.variantId, key: lineItem.key, currentQuantity: lineItem.quantity, currentProps: lineItem.properties };
    return false;
  ...
  async submitFormObject(formObj, args) {
    if (args.type == 'submitItem') {
      this.getLineItem({variantId: formObj.variantId}).then(resp=> {
        if (resp==false) {
           this.addCartItem(formObj);
        } else {
          const newObj = {
            quantity: resp.currentQuantity + formObj.quantity,
            properties: Object.assign({}, resp.properties, formObj.properties)
          }
  ...
  async addCartItem(args) {
    ...
    cart.addItem(args.variantId, addObj).then(resp => { 
      this.renderUpdates()
    }).catch(err=> {
  ...
  async updateCartItem(lineItem, args) {  
    if (args.quantity === 0) {
      return this.removeCartItem(Object.assign({}, args, {variantId: lineItem.variantId}));
    }
    return cart.updateItem(lineItem.key,args).then(resp=> {
      var updatedItem = resp.items.find(item=>{return item.variant_id == lineItem.variantId})
      if (!updatedItem) {
      ...
      this.renderUpdates();
      var returnObj = { quantity: updatedItem.quantity, properties: updatedItem.properties }
      return returnObj;
    }).catch(err => {
  ...
  async updateCartNote(note) {
   ...
   async getCartState(updateState = true) {
    ...
    var cartState = await cart.getState().then(state => {
      return state
    }).catch(err=>{
      console.error(err)
      return false;
...
renderSavedText() {
  ...
  setupBroadcast() {
    ...
    this.broadcastDismiss.addEventListener('click', this.closeBroadcast.bind(this))
  }
  ...
  broadcastRenderLoop(message, error = false) {
    ...
    this.broadcastEl.style.maxHeight = this.broadcastMessage.getBoundingClientRect().height + 'px';
  ...
  handleCartError(type, error) {
    ...
    setTimeout(() => {
      ...
      if (pres.length) {
        pres.forEach(pre => {pre.classList.remove(classes.itemPre) })
      }
    }, 200);
    
    if (typeof error == 'string') {
     return  this.broadcast({ type: 'error', message: errorTexts.en[type][error] })
    }
    error.json().then(resp=> { 
      if (resp.description.indexOf("can't add more") >= 0) {
        this.broadcast({ type: 'error', message: errorTexts.en[type]['notEnough']})
      } else {
        this.broadcast({ type: 'error', message: errorTexts.en[type]['default'] })
      }
    })
  }
